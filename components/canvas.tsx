...
import { broadcastCursorThrottled } from "@/lib/cursorSync"
...
export function Canvas() {
  ...
  const [cursorMode, setCursorMode] = useState<'default' | 'move' | 'rotate' | 'resize' | 'crosshair'>('default')
  ...
  const cursorColor = (isOnline && currentUser && currentUser.isAdmin) ? "#FFD700" : (isOnline && currentUser) ? currentUser.color : "#71717a" 
  ...
  // Re-render trigger
  useEffect(() => { renderCanvas() }, [objects, zoom, panX, panY, selectedIds, layers, showGrid, gridType, highlightColor, interactionMode, canvasMousePos, theme, canvasSettings, currentPath, showTransformHandles, editingTextId])

  // Keyboard shortcuts
  useKeyboardShortcuts()

  // Broadcast cursor when tool changes
  useEffect(() => {
    if (collaboration && canvasMousePos) {
      collaboration.broadcastCursor(canvasMousePos);
    }
  }, [activeTool, collaboration, canvasMousePos]);
  ...
  const getClickType = (point: Point, obj: CanvasObject) => {
    if (!showTransformHandles || !selectedIds.includes(obj.id)) return isPointInsideObject(point, obj) ? "body" : "none"
    ...
    const edges = [
      { x: width / 2, y: 0, name: "n" },  // Top
      { x: width, y: height / 2, name: "e" },  // Right
      { x: width / 2, y: height, name: "s" }, // Bottom
      { x: 0, y: height / 2, name: "w" }   // Left
    ]

    // Check Corner Resize Handles (highest priority)
    ...
    for (const corner of corners) {
        ...
        if (rotatedX >= corner.x - rZoneX && rotatedX <= corner.x + rZoneX &&
            rotatedY >= corner.y - rZoneY && rotatedY <= corner.y + rZoneY) {
          return "rotate"
        }
    }
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (clickedObject) {
      ...
      } else if (['nw', 'ne', 'sw', 'se', 'n', 'e', 's', 'w'].includes(clickType)) {
        setSelectedIds([clickedObject.id])
        initialObjectStateRef.current = clickedObject
    ...
    // Empty click - Tool specific actions
    if (activeTool === 'text') {
      // Open text editing modal - no object created yet
      setTextPosition(point)
      setTextEditingId('new')
      return
    } else if (activeTool === 'note') {
      // Create new sticky note object with random pastel color
      const pastelColors = ['#FFFF88', '#FFCCCC', '#CCFFCC', '#CCCCFF', '#FFCCFF', '#FFFFCC', '#FFE4B5', '#E6E6FA']
      ...
      setNoteEditingId(noteObj.id)
      // Auto-switch to selection tool after placing note
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (collaboration) broadcastCursorThrottled(point)
    ...
    if (mode === 'none') {
        ...
        if (activeTool === 'select') {
            const hovered = [...objects].reverse().find(obj => {
               ...
               return l?.visible && !l?.locked && getClickType(point, obj) !== 'none'
            });
            
            if (hovered && selectedIds.includes(hovered.id)) {
                const type = getClickType(point, hovered)
                if (type === 'rotate') hoverMode = 'crosshair'
                else if (type === 'nw' || type === 'se') hoverMode = 'nw-resize'
                else if (type === 'ne' || type === 'sw') hoverMode = 'ne-resize'
                else if (type === 'n' || type === 's') hoverMode = 'ns-resize'
                else if (type === 'e' || type === 'w') hoverMode = 'ew-resize'
        ...
        if (mode === 'rotating') setCursorMode('crosshair')
        else if (mode === 'resizing') {
            const activeHandle = activeHandleRef.current
            if (activeHandle === 'nw' || activeHandle === 'se') setCursorMode('nw-resize')
            else if (activeHandle === 'ne' || activeHandle === 'sw') setCursorMode('ne-resize')
            else if (activeHandle === 'n' || activeHandle === 's') setCursorMode('ns-resize')
            else if (activeHandle === 'e' || activeHandle === 'w') setCursorMode('ew-resize')
            else setCursorMode('resize')
        }
        else if (mode === 'moving') setCursorMode('grabbing')
    ...
    if (mode === 'drawing') {
       if (activeTool === 'eraser') handleEraser(point)
          ...
          if (e.shiftKey && currentPath.length > 0) {
            ...
            const start = currentPath[0]; const dx = Math.abs(point.x - start.x); const dy = Math.abs(point.y - start.y); 
            if (dx > dy) newPoint = { x: point.x, y: start.y }; else newPoint = { x: start.x, y: point.y }
          }
    ...
    if (mode === 'moving' && dragStartRef.current) { 
        ...
        initialObjectsMapRef.current.forEach((initObj, id) => { 
            if (objects.some(o => o.id === id)) { 
                updateObject(id, { transform: { ...initObj.transform, x: initObj.transform.x + dx, y: initObj.transform.y + dy } }) 
            } 
        }) 
    }
    ...
    if (mode === 'rotating' && initialObjectStateRef.current) { 
        ...
        updateObject(obj.id, { transform: { ...obj.transform, rotation: angle } }) 
    }
    ...
    if (mode === 'resizing' && initialObjectStateRef.current && activeHandleRef.current && dragStartRef.current) { 
        ...
        // Shift: Free (Distort)
        if (!e.shiftKey) {
           const startRatio = obj.transform.scaleX / obj.transform.scaleY
           const currentRatio = scaleX / scaleY
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    if (mode === 'selecting' && dragStartRef.current) { 
        const x1 = Math.min(dragStartRef.current.x, point.x); const y1 = Math.min(dragStartRef.current.y, point.y); 
        const x2 = Math.max(dragStartRef.current.x, point.x); const y2 = Math.max(dragStartRef.current.y, point.y); 
        const ids: string[] = []; 
        objects.forEach(obj => { 
            const layer = layers.find(l => l.id === obj.layerId); if (!layer || !layer.visible || layer.locked) return; 
            const { width, height } = getObjectSize(obj); 
            const ox = obj.transform.x; const oy = obj.transform.y; 
    ...
    if (mode === 'drawing' && activeTool === 'brush' && currentPath.length > 1) { 
        ...
        addObject({ id: `path-${Date.now()}`, name: t("brush", "tools"), type: "path", layerId: activeLayerId, transform: { x: minX, y: minY, rotation: 0, scaleX: 1, scaleY: 1, anchor: "center" }, data: { points: norm, strokeColor: brushSettings.color, strokeWidth: brushSettings.size, opacity: brushSettings.opacity } }); 
        setCurrentPath([]) 
    }

    if (mode === 'drawing' && activeTool === 'shape' && startPointRef.current) { 
        const rawW = point.x - startPointRef.current.x; const rawH = point.y - startPointRef.current.y; 
        const w = Math.abs(rawW); const h = Math.abs(rawH); 
        const x = rawW < 0 ? point.x : startPointRef.current.x; 
        const y = rawH < 0 ? point.y : startPointRef.current.y; 
        if (w > 5 && h > 5) { 
            addObject({ id: `shape-${Date.now()}`, name: t("shape", "tools"), type: "shape", layerId: activeLayerId, transform: { x, y, rotation: 0, scaleX: 1, scaleY: 1, anchor: "center" }, data: { shapeType: shapeSettings.shapeType, width: w, height: h, fillColor: shapeSettings.fillColor, strokeColor: shapeSettings.strokeColor, strokeWidth: shapeSettings.strokeWidth, borderType: shapeSettings.borderType, opacity: shapeSettings.opacity } }); 
            setActiveTool('select') 
        } 
        setStartPoint(null) 
    }
  ...
  const handleDoubleClick = (e: React.MouseEvent) => { 
    const point = getCanvasPoint(e); 
    const clickedObject = [...objects].reverse().find((obj) => isPointInsideObject(point, obj)); 
  ...
  const renderCanvas = () => {
    ...
    [...layers].reverse().forEach(layer => {
      ...
      [...layer.objectIds].reverse().forEach(objId => {
        ...
        ctx.save(); ctx.translate(obj.transform.x, obj.transform.y)
    ...
    // Selection Rect
    if (interactionMode === 'selecting' && dragStartRef.current && canvasMousePos) {
    ...
    // Brush Preview
    if (isOnline && (activeTool === 'brush' || activeTool === 'eraser') && canvasMousePos) {
      ctx.save(); ctx.beginPath(); ctx.arc(canvasMousePos.x, canvasMousePos.y, brushSettings.size / 2, 0, Math.PI * 2)
  ...
  const renderNote = (ctx: CanvasRenderingContext2D, obj: CanvasObject) => {
    ...
    const cornerRadius = data.cornerRadius || 8;

    // Draw rounded rectangle background
    ctx.fillStyle = data.backgroundColor || '#FFFF88';
    ctx.beginPath();
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 w-full h-full overflow-hidden">
        <canvas
            ...
            style={{
                cursor: cursorMode === 'move' ? 'move' :
                       cursorMode === 'grabbing' ? 'grabbing' :
                       cursorMode === 'nw-resize' ? 'nw-resize' :
                       cursorMode === 'ne-resize' ? 'ne-resize' :
                       cursorMode === 'ns-resize' ? 'ns-resize' :
                       cursorMode === 'ew-resize' ? 'ew-resize' :
                       cursorMode === 'crosshair' ? 'crosshair' :
                       cursorMode === 'text' ? 'text' : 'default'
            }}
        ...
        {/* Text Editor Modal */}
        {textEditingId && textPosition && (
          <TextEditorOverlay
            position={textPosition}
            initialContent={textEditingId === 'new' ? '' : (objects.find(o => o.id === textEditingId)?.data as TextData)?.content || ''}
            isAreaText={false}
            onCommit={(content, properties) => {
              if (textEditingId === 'new') {
                const textObj: CanvasObject = {
                  id: `text-${Date.now()}`,
                  type: 'text',
                  transform: { x: textPosition.x, y: textPosition.y, rotation: 0, scaleX: 1, scaleY: 1, anchor: 'top-left' },
                  data: { content, ...properties, width: 200, height: 50 }
                }
                addObject(textObj)
                setSelectedIds([textObj.id])
                setActiveTool('select') // Auto-switch to selection tool
                if (collaboration) collaboration.broadcastTextCommit(textObj)
              } else {
                const obj = objects.find(o => o.id === textEditingId)
                if (obj) {
                  updateObject(textEditingId, { data: { ...obj.data, content, ...properties } })
                  if (collaboration) collaboration.broadcastTextCommit({ ...obj, data: { ...obj.data, content, ...properties } })
                }
              }
            ...
            onLiveUpdate={(content) => {
              if (textEditingId !== 'new' && collaboration) {
                collaboration.broadcastTextLive(textEditingId, content)
              }
            }}
          />
        )}
        ...
        {/* Note Editor Overlay */}
        {noteEditingId && notePosition && (
          <NoteEditorOverlay
            position={notePosition}
            initialContent={noteEditingId === 'new' ? '' : (objects.find(o => o.id === noteEditingId)?.data as NoteData)?.content || ''}
            onCommit={(content) => {
              if (noteEditingId === 'new') {
                const pastelColors = ['#FFFF88', '#FFCCCC', '#CCFFCC', '#CCCCFF', '#FFCCFF', '#FFFFCC', '#FFE4B5', '#E6E6FA']
                const randomColor = pastelColors[Math.floor(Math.random() * pastelColors.length)]
                const noteObj: CanvasObject = {
                  id: `note-${Date.now()}`,
                  type: 'note',
                  transform: { x: notePosition.x, y: notePosition.y, rotation: 0, scaleX: 1, scaleY: 1, anchor: 'top-left' },
                  data: { content, backgroundColor: randomColor, backgroundType: 'plain', fontFamily: 'Inter', fontSize: 14, width: 200, height: 120 }
                }
                addObject(noteObj)
                setSelectedIds([noteObj.id])
                if (collaboration) collaboration.broadcastNoteCreate(noteObj)
              } else {
                const obj = objects.find(o => o.id === noteEditingId)
                if (obj) {
                  updateObject(noteEditingId, { data: { ...obj.data, content } })
                }
              }
